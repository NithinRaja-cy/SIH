...
export default function Reports({ currentSessionData }) {
  const sessions = currentSessionData ? [
    {
      id: currentSessionData.session_id,
      date: currentSessionData.timestamp || 'Just now',
      duration: `${currentSessionData.duration_seconds || 2.0}s`,
      riskScore: currentSessionData.risk?.risk_score || 0,
      riskLevel: currentSessionData.risk?.risk_level || 'LOW',
      decision: currentSessionData.risk?.final_decision || 'AUTHENTIC SPEECH VERIFIED (LOW RISK)',
      speakerMatch: `${currentSessionData.speaker?.speaker_similarity || 78}%`,
      syntheticProb: `${currentSessionData.deepfake?.synthetic_probability || 10}%`
    }
  ] : [];
      ...
      <div className="light-card overflow-hidden">
        {sessions.length > 0 ? (
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      <span className={`px-2.5 py-0.5 rounded-full font-black ${
                        item.riskLevel === 'HIGH' ? 'bg-rose-100 text-rose-800' : (item.riskLevel === 'MEDIUM' ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800')
                      }`}>
                        {item.riskScore} / 100
                      </span>
                    </td>
          ...
          <div className="p-8 text-center text-slate-500 text-xs font-semibold">
            No active session reports generated yet. Upload an audio file and run analysis to view reports.
          </div>
        )}
      </div>
